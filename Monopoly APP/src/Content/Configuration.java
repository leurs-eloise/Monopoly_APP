...
public class Configuration {
	...
	public boolean loadConfig(String file) { // Config perso
		try (BufferedReader br = new BufferedReader(new FileReader(file))) {
			...
			try {
				...
				stringToSend
						.receiveMsg("[Erreur] Parametre case mal renseigne. Tentative prise de valeur par defaut (20)");
			}
			try {
				nbJoueur = gameSetting.getInt("player");
			} catch (Exception e) {
				stringToSend.receiveMsg(
						"[Erreur] Parametre player mal renseigne. Tentative prise de valeur par defaut (2)");
			}
			try {
					...
					if (type.equals("depart")) {
						...
						try {
							...
							if (caseInfo.getJSONArray("loyer").length() < lvlMaxProp) {
								erreur += "\n[Erreur] " + lvlMaxProp + " prix sont exiges pour le loyer sur la case "
										+ i;
							}
			...
			for (JSONObject caseInfo : listeGare) { // verif loyer gare
				if (caseInfo.getJSONArray("loyer").length() < nbGare) {
					stringToSend.receiveMsg("[Erreur] Gare " + caseInfo.getString("nom") + " doit avoir au moins "
							+ nbGare + " loyer.");
					erreurBoolean = true;
				}
			}
	...
	public ArrayList<Integer> getLoyer(int i) {
		...
		ArrayList<Integer> listeLoyer = CaseFactory.getInstance()
				.JSONArrayToIntArrayList(listeCaseJSON.get(i).getJSONArray("loyer"));
		return listeLoyer;
	}

	public ArrayList<Integer> getMultiplicateur(int i) {
		ArrayList<Integer> multiplicateur = CaseFactory.getInstance()
				.JSONArrayToIntArrayList(listeCaseJSON.get(i).getJSONArray("multiplicateur"));
