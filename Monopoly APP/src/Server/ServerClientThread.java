			...
			while (!clientMessage.equals("disconnect")) {
				...
				if (clientMessageSplit[0].equals("setname")) {
					...
					if (Partie.getInstance().lancerDes()) {
						serverMessage = "[Info] D�s lanc�";
					} else {
						serverMessage = "[Erreur] Erreur lors du lancement des d�s";
					}
				}
					...
					if (clientMessageSplit.length == 2) {
						try {
							...
							if (level < 0 || level > 6) {
								serverMessage = "[Erreur] Le niveau doit �tre compris entre 0 et 5";
							} else {
								for (Case item : Configuration.getInstance().getListeCase()) {
							...
							serverMessage = "[Erreur] Le niveau doit �tre un nombre";
						}
					}
					...
					if (Partie.getInstance().init()) {
						serverMessage = "[Info] Partie d�mar�e" + "\nA "
